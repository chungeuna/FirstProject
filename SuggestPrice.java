...
public class SuggestPrice {
	...
	public void suggestMoney () {
		...
		if (!personManager.getPersonList().contains(scanPhoneNumber)) {
			...
			for (int i=0; i<personManager.getPersonList().size(); i++) {
				if (personManager.getPersonList().get(i).equals(scanPhoneNumber)) {	
					for (int j=0; j<person.eventlist.size(); j++) {
						getMoneyList.add(person.eventlist.get(j).getMoney);
					}	
				}
			}
			...
			for (int i=0; i<personManager.getPersonList().size(); i++) {
				if (personManager.getPersonList().get(i).equals(scanPhoneNumber)) {
					for (int j=0; j<person.eventlist.size(); j++) {
						spendMoneyList.add(person.eventlist.get(j).spendMoney);
					}	
				}
			}	
			...
			int length = (int)(Math.log10(payMoney)+1); 		// 숫자의 자릿수 십의 자릿수만큼 끊음-만원이면 천원단위까지 끊고 십만 단위면 만원단위까지 끊으면?
